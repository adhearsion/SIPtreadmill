...
describe TestRunner do
  ...
  context "when the test run has a receiver scenario" do
    ...
    let(:receiver_sippy) { receiver_scenario.to_scenario_object receiver_options }
    let(:receiver_runner) { double run: {} }

    it "starts the receiver scenario before the main scenario" do
      receiver_scenario.should_receive(:to_scenario_object).twice.with(receiver_options).ordered.and_return(receiver_sippy)
    ...
    context "with CSV data" do
      ...
      it "writes the receiver scenario csv data and passes it to SippyCup" do
        ...
        receiver_scenario.should_receive(:to_scenario_object).twice.with(receiver_options).and_return(receiver_sippy)
        SippyCup::Runner.should_receive(:new).with(receiver_sippy, full_sipp_output: false).ordered.and_return(receiver_runner)
        receiver_runner.should_receive :run
    ...
    context "and a registration scenario" do
      ...
      it "writes the receiver scenario to disk and starts it before starting the main scenario" do
        ...
        reg_sippy = registration_scenario.to_scenario_object reg_options
        mock_reg_runner = double 'SippyCup::Runner'
        registration_scenario.should_receive(:to_scenario_object).with(reg_options).ordered.and_return reg_sippy
